...
export default function PlansPage() {
  const {
    ...
    useShallow((state) => ({
      ...
      addSmartPlan: state.addSmartPlan,
      updateSmartPlan: state.updateSmartPlan,
      deleteSmartPlan: state.deleteSmartPlan,
    })),
  );
  ...
  const openEdit = (plan: SmartPlan) => {
    ...
    setForm({
      ...
      recitationAmount: plan.recitationAmount,
      notes: plan.notes || "",
    });
    setError("");
    setShowForm(true);
  ...
  const savePlan = async (event: FormEvent) => {
    ...
    if (form.newAmount < 1 || form.reviewAmount < 1 || form.recitationAmount < 1 || form.endDate < form.startDate) {
    ...
    try {
      if (editing) {
        await updateSmartPlan(editing.id, {
          ...
          recitationAmount: form.recitationAmount,
          notes: form.notes,
        });
      } else {
        await addSmartPlan({
  ...
  const changeAmount = async (
    plan: SmartPlan,
    field: "newAmount" | "reviewAmount" | "recitationAmount",
    delta: number
  ) => {
  ...
  const printPlan = (plan: SmartPlan, cashier: boolean) => {
    ...
    if (!student) return;
    const revisionOnly = student.status === "graduated";
    const days: string[] = [];
    ...
    const rows = days
      .map(
        (date) => `<tr><td>${date}</td>${revisionOnly ? "" : `<td>${plan.newAmount} ${UNIT_LABEL[plan.unit]}</td>`}<td>${plan.reviewAmount} ${UNIT_LABEL[plan.unit]}</td><td>${plan.recitationAmount} ${UNIT_LABEL[plan.unit]}</td><td>□</td></tr>`
      )
      .join("");
    ...
    popup.document.write(`<!doctype html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><title>خطة ${student.name}</title><style>
      ...
      *{box-sizing:border-box} body{font-family:Tajawal,Arial,sans-serif;color:#111;margin:0;${cashier ? "width:72mm;font-size:10px" : "font-size:14px"}}
      ...
      </style></head><body><h1>${revisionOnly ? "خطة المراجعة والسرد" : "خطة الحفظ والمراجعة والسرد"} ${PERIOD_LABEL[plan.period]}</h1><p>${student.name}</p><div class="info">من ${plan.startDate} إلى ${plan.endDate}<br>${revisionOnly ? "" : `الحفظ: ${plan.newAmount} ${UNIT_LABEL[plan.unit]} — `}المراجعة: ${plan.reviewAmount} ${UNIT_LABEL[plan.unit]} — السرد: ${plan.recitationAmount} ${UNIT_LABEL[plan.unit]}</div><table><thead><tr><th>اليوم</th>${revisionOnly ? "" : "<th>الحفظ</th>"}<th>المراجعة</th><th>السرد/التلاوة</th><th>تم</th></tr></thead><tbody>${rows}</tbody></table>${plan.notes ? `<div class="notes">ملاحظات: ${escapeHtml(plan.notes)}</div>` : ""}<div class="sign"><span>المعلم: ______</span><span>ولي الأمر: ______</span></div><script>onload=()=>print()</script></body></html>`);
  ...
  const formStudent = students.find((student) => student.id === form.studentId);
  const formRevisionOnly = formStudent?.status === "graduated";
    ...
    <div className="space-y-6 pb-20">
      ...
      <section className="space-y-4">
        {visiblePlans.map((plan) => {
          ...
          return <article key={plan.id} className="bg-[var(--surface)] border border-gray-200 dark:border-gray-800 rounded-3xl p-5 space-y-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><h2 className="text-xl font-black text-[var(--foreground)]">{student?.name || "طالب غير متاح"}</h2><p className="text-xs text-gray-500 mt-1">{PERIOD_LABEL[plan.period]} · {plan.startDate} — {plan.endDate}</p></div>
            ...
            {revisionOnly ? <div className="rounded-xl bg-amber-50 dark:bg-amber-950/20 px-4 py-3 text-sm font-bold text-amber-800 dark:text-amber-300">خاتم للقرآن · مسار مراجعة وسرد/تلاوة فقط</div> : <AmountRow label="الحفظ اليومي" value={plan.newAmount} unit={UNIT_LABEL[plan.unit]} enabled={plan.status === "active"} onMinus={() => changeAmount(plan,'newAmount',-1)} onPlus={() => changeAmount(plan,'newAmount',1)} />}
            ...
            <AmountRow label="السرد/التلاوة اليومية" value={plan.recitationAmount} unit={UNIT_LABEL[plan.unit]} enabled={plan.status === "active"} onMinus={() => changeAmount(plan,'recitationAmount',-1)} onPlus={() => changeAmount(plan,'recitationAmount',1)} />
      ...
      {showForm && <div className="fixed inset-0 z-50 bg-black/50 flex items-end md:items-center justify-center p-0 md:p-4"><div className="bg-[var(--surface)] rounded-t-3xl md:rounded-3xl w-full max-w-2xl max-h-[92vh] overflow-y-auto p-6"><div className="flex justify-between items-center mb-5"><h2 className="text-xl font-black">{editing ? "تعديل الخطة" : "خطة جديدة"}</h2><button onClick={() => setShowForm(false)}><X /></button></div><form onSubmit={savePlan} className="space-y-5">
        ...
        <label className="block"><span className="block text-sm font-bold mb-2">وحدة المقرر</span><select value={form.unit} onChange={(event) => setForm({...form,unit:event.target.value as PlanUnit})} className="w-full p-3 rounded-xl border bg-white dark:bg-gray-800 dark:border-gray-700">{(['ayahs','pages','lines','hizbs'] as PlanUnit[]).map((unit) => <option key={unit} value={unit}>{UNIT_LABEL[unit]}</option>)}</select></label>
        {formRevisionOnly ? <div className="rounded-xl border border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/20 p-4 text-sm font-bold text-amber-800 dark:text-amber-300">هذا الطالب خاتم للقرآن؛ الخطة مخصصة للمراجعة والسرد/التلاوة فقط.</div> : <Stepper label={`الحفظ اليومي (${UNIT_LABEL[form.unit]})`} value={form.newAmount} onChange={(value) => setForm({...form,newAmount:value})} />}
        ...
        <Stepper label={`السرد/التلاوة اليومية (${UNIT_LABEL[form.unit]})`} value={form.recitationAmount} onChange={(value) => setForm({...form,recitationAmount:value})} />
...
function Stat({label,value,color}:{label:string;value:number;color:string}) { return <div className="bg-[var(--surface)] border border-gray-200 dark:border-gray-800 rounded-2xl p-4 text-center"><strong className={`text-2xl ${color}`}>{value}</strong><p className="text-xs text-gray-500 mt-1">{label}</p></div>; }
