...
function googleAuthErrorMessage(error: unknown): string {
  ...
  if (normalized.includes('provider is not enabled') || normalized.includes('unsupported provider')) {
    return 'تسجيل Google غير مفعّل في مشروع Supabase. فعّل Google Provider وأدخل Client ID وClient Secret ثم أضف عناوين التحويل المسموحة.';
  }
  if (normalized.includes('redirect') || normalized.includes('callback')) {
    return 'إعداد تحويل Google غير مطابق. راجع Callback URL في Google وRedirect URLs في Supabase.';
  }
  return message || 'تعذر تسجيل الدخول باستخدام Google';
}
...
export default function AuthPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setConfigurationError(googleAuthErrorMessage(error));
      setLoading(false);
    }
  };
