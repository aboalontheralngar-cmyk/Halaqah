...
export function StatCard({ label, value, icon: Icon, trend, color }: StatCardProps) {
  ...
  return (
    <div className="bg-[var(--surface)] p-8 rounded-3xl border border-[var(--border)] shadow-xl shadow-gray-200/20 dark:shadow-none group hover:-translate-y-1 transition-all duration-500">
      ...
      <h3 className="text-4xl font-black text-[var(--foreground)] tracking-tight">{value}</h3>
    </div>
  );
}
...
export function ActionButton({ label, icon: Icon, color, onClick }: ActionButtonProps) {
  return (
    <button 
      onClick={onClick}
      className="group flex flex-col items-center justify-center p-8 bg-[var(--surface)] rounded-3xl border border-[var(--border)] shadow-lg shadow-gray-200/20 dark:shadow-none hover:shadow-2xl hover:shadow-teal-100 dark:hover:shadow-none transition-all duration-500 active:scale-95"
    >
