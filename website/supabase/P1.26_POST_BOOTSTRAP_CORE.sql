-- P1.26 post-bootstrap core schema completion.
-- Prepared against the owner-provided schema after P1.26_GUARDED_BASE_BOOTSTRAP.sql
-- completed successfully. The public schema also contains unrelated operational
-- tables from another workload; this script deliberately never alters or deletes
-- those tables. It only creates/repairs the Halaqah tables listed below.
--
-- Preconditions confirmed by the provided schema: profiles/supervisors/centers/
-- halaqat/center_members/families/family_guardians exist with UUID scope keys;
-- students and the Halaqah core/learning surfaces are still absent.
--
-- Safety: additive/idempotent, transactional, no DROP TABLE, TRUNCATE, or row
-- DELETE. If the base scope is missing or has incompatible identifier types the
-- transaction aborts before creating Halaqah core tables.
...
BEGIN
  ...
  IF to_regclass('public.family_guardians') IS NULL THEN missing_base := array_append(missing_base, 'family_guardians'); END IF;

  IF cardinality(missing_base) > 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'P1.26_POST_BOOTSTRAP_BASE_MISSING',
      DETAIL = 'Missing Halaqah base scope tables: ' || array_to_string(missing_base, ', '),
      HINT = 'Stop. Do not create core tables until the Halaqah base scope is complete.';
  END IF;
END;
$$;
...
BEGIN
  WITH required_scope(table_name, column_name, expected_type) AS (
    VALUES
      ...
      ('family_guardians','id','uuid'), ('family_guardians','family_id','uuid'),
      ('family_guardians','center_id','uuid'), ('family_guardians','halaqa_id','uuid')
  )
  SELECT
  ...
  IF missing_scope_columns > 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'P1.26_POST_BOOTSTRAP_SCOPE_COLUMNS_MISSING',
      DETAIL = missing_scope_columns::TEXT || ' required center/halaqa/member/family column(s) are missing.',
      HINT = 'Stop and send the P1.26 post-bootstrap core error. This migration will not invent or rewrite the authorization scope.';
  END IF;
  IF bad_scope_types > 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'P1.26_POST_BOOTSTRAP_SCOPE_TYPE_MISMATCH',
      DETAIL = bad_scope_types::TEXT || ' required scope column(s) use an incompatible type.',
      HINT = 'Stop and send the P1.26 post-bootstrap core error. Existing identifiers and roles will not be coerced.';
  END IF;
END;
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'P1.26_POST_BOOTSTRAP_SCOPE_KEY_TYPE_MISMATCH',
      DETAIL = 'centers.id, halaqat.id and halaqat.center_id must use UUID keys.',
      HINT = 'Stop and send the P1.26 post-bootstrap core error. This repair does not coerce or replace existing identifiers.';
  END IF;
  ...
  IF mismatch_count > 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'P1.26_POST_BOOTSTRAP_REFERENCE_KEY_TYPE_MISMATCH',
      DETAIL = mismatch_count::TEXT || ' existing reference column(s) are not UUID.',
      HINT = 'Stop and send the P1.26 post-bootstrap core error. The repair never changes identifier types.';
  END IF;
END;
$$;
...
-- contain richer role logic, so the compatibility layer only supplies a conservative fallback
