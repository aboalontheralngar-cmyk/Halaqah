-- P1.27 Build 82 Hotfix 7 targeted STUDENT PORTAL repair.
-- Use ONLY if P1.27_BUILD82_HOTFIX7_VERIFY.sql reports portal contract failures.
-- This intentionally does NOT replay P7.3 and does NOT replace the shared
-- current_user_* scope helpers installed by the current Build 75/76 path.
...
BEGIN
  IF to_regprocedure('public.current_user_can_access_halaqa(uuid,uuid)') IS NULL THEN
    RAISE EXCEPTION 'current supervision/scope contract missing; repair Build 75/76 first';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'students' AND column_name = 'student_code'
  ) THEN
    RAISE EXCEPTION 'students.student_code missing; P7 identity foundation is required first';
  END IF;
END;
$$;
