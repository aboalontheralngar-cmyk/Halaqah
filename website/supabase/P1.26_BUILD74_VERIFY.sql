-- P1.26 Build 74 read-only readiness verification.
-- Expected result: every returned row has passed = true.

WITH required_supervision_functions(signature) AS (
  VALUES
...
), required_atomic_functions(signature) AS (
  VALUES
  ...
  SELECT 'schema'::text AS check_group, 'supervision_tables_exist'::text AS check_name,
    ...
    'supervision tables'::text AS details

  UNION ALL
  SELECT 'rpc', 'supervision_rpcs_exist',
    NOT EXISTS (SELECT 1 FROM required_supervision_functions WHERE to_regprocedure(signature) IS NULL),
    coalesce((SELECT string_agg(signature, ', ') FROM required_supervision_functions WHERE to_regprocedure(signature) IS NULL), 'all present')

  UNION ALL
  SELECT 'rpc', 'atomic_family_rpcs_exist',
    NOT EXISTS (SELECT 1 FROM required_atomic_functions WHERE to_regprocedure(signature) IS NULL),
    coalesce((SELECT string_agg(signature, ', ') FROM required_atomic_functions WHERE to_regprocedure(signature) IS NULL), 'all present')

  UNION ALL
  SELECT 'membership', 'membership_pair_unique',
    EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND tablename='supervisor_members' AND indexname='uq_supervisor_members_supervisor_user'),
    'unique supervisor_id + user_id'

  UNION ALL
  SELECT 'membership', 'no_duplicate_memberships',
    NOT EXISTS (SELECT 1 FROM public.supervisor_members GROUP BY supervisor_id, user_id HAVING count(*) > 1),
    'duplicates must be zero'

  UNION ALL
  SELECT 'membership', 'owners_reconciled',
    NOT EXISTS (
    ...
    'every owner_id has active owner membership'

  UNION ALL
  SELECT 'membership', 'single_active_owner',
    NOT EXISTS (
  ...
  UNION ALL
  SELECT 'rls', 'supervision_rls_enabled',
    NOT EXISTS (
      SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
      WHERE n.nspname='public'
        AND c.relname IN ('supervisor_members','supervisor_center_invitations','supervisor_member_invitations','supervisor_audit_events','supervision_visits')
  ...
  SELECT 'security', 'protected_search_path',
    ...
    'Build 74 SECURITY DEFINER functions pin search_path including pg_temp'

  UNION ALL
  SELECT 'auth', 'supervisor_enum_role_available',
    EXISTS (
